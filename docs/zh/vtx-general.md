...
- P1MW ：此模式下，射频输出功率为 0.1mW（以避免干扰其他飞手）。使用VTX菜单退出P1MW模式。

- 0MW ：此模式下射频输出为零。使用遥控器快捷键退出0MW模式。
...
用于将射频输出功率微调至精确的 25mW。范围为 [-10:10]，步长约为每步0.25 dB。
...
- HDZero VTX 即使在 1mW 模式下仍会发热。当 VTX 需要一段时间闲置时，最好将其保持在 0mW 模式，虽然在此模式下接收器无法接收实时视频。
...
- 快捷方式：OPT_A
  ...
  - VTX 启动时，功率为 0mW。当使用摇杆指令 (OPT-A \ /) 退出 0mW时，VTX 才会开始发射。
